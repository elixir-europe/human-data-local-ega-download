 ...
 * @author asenf
 * 
 * Separate out reading into a Thread; to better separate different tasks
 * Developed to separate re-encryption (which happens in 512-byte blocks) from
 * Data transfer (which happens in large chunks, but potentially slowly due to 
 * connection speeds). Can be used for any input stream.
 */

public class MyNewBackgroundInputStream extends InputStream {
    ...
    public MyNewBackgroundInputStream(InputStream source) {
        this(source, QUEUE_SIZE, BUFFER_SIZE);
    }

    public MyNewBackgroundInputStream(InputStream source, int queueSize, int bufferSize)
    {
